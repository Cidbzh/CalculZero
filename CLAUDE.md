...
2. **HTML** — `#home` (basculeur matière `data-mat` « Maths / Physique-Chimie / Allemand », niveau, tuiles thèmes `#subGrid`, entraînement libre, sprint 60 s, stats, `#statsExtra` injecté en JS, boutons thème `data-theme-pick`) et `#play` (zone question `#qbox`).
...
- **Thème (Auto / Clair / Sombre)** : `applyTheme(t, animate)` pose/retire l'attribut `data-theme` sur `<html>`, persiste sous `cz_theme` (`"auto"`|`"light"`|`"dark"`), met à jour `.on` des boutons `data-theme-pick`, et ajoute `html.theming` le temps d'une transition douce (≈ 520 ms) ; `"auto"` = attribut retiré → on suit le système. Le **script anti-flash** en tête réapplique le choix avant le premier rendu. Toute nouvelle animation reste désactivée sous `prefers-reduced-motion`.
